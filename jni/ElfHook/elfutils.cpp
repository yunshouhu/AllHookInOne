...
void getElfInfoBySegmentView(ElfInfo &info, const ElfHandle *handle){
	...
	info.handle = handle;
	info.elf_base = (uint8_t *) handle->base;
	info.ehdr = reinterpret_cast<Elf32_Ehdr *>(info.elf_base);//从基址强制转换获取Elf32_Ehdr

	// may be wrong
	info.shdr = reinterpret_cast<Elf32_Shdr *>(info.elf_base + info.ehdr->e_shoff);//强制转换获取Elf32_Shdr
	info.phdr = reinterpret_cast<Elf32_Phdr *>(info.elf_base + info.ehdr->e_phoff);//强制转换获取Elf32_Phdr
